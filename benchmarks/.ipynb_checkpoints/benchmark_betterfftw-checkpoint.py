...
def create_enhanced_summary_report(results_dict, save_file=None):
    ...
    if 'direct_comparison' in results_dict:
        ...
        if 'implementation' in df.columns:
            # Check if we have 'avg_execution_time' or 'time' column
            time_col = 'avg_execution_time' if 'avg_execution_time' in df.columns else 'time'
            
            if time_col in df.columns:
                # Check for required columns before creating pivot
                index_cols = ['dimension', 'transform', 'size']
                # Only include dataset_idx if it exists
                if 'dataset_idx' in df.columns:
                    index_cols.append('dataset_idx')
                ...
                try:
                    ...
                    if 'NumPy FFT' in pivot.columns:
                        ...
                        # Add analysis for power-of-2 vs non-power-of-2 sizes if the column exists
                        if 'is_power_of_2' in df.columns:
                            ...
                            pow2_analysis = df.groupby(['implementation', 'dimension', 'is_power_of_2'])[time_col].mean().unstack()
                    ...
                    report.append(f"Error analyzing direct comparison data: {str(e)}\n")

    # Rest of the function remains the same...
    # (include the remaining parts of the function here)
    ...
    if 'real_world' in results_dict:
        ...
        if 'implementation' in df.columns and 'avg_time' in df.columns:
            try:
                        ...
                        for impl in scenario_df['implementation'].unique():
                            if impl != 'NumPy FFT':
                                ...
                                try:
                                    ...
                                    # Handle potential errors in t-test (e.g., not enough samples)
                                    row = [
                                        scenario,
                                        f"NumPy vs {impl}",
                                        f"Error: {str(e)}",
                ...
                report.append(f"Error analyzing real-world scenario data: {str(e)}\n")
            ...
            for impl, impl_df in df.groupby('implementation'):
                ...
                if not max_thread_df.empty:
                    try:
                        ...
                        report.append(f"Error analyzing thread scaling data for {impl}: {str(e)}\n")
    ...
    if 'memory_efficiency' in results_dict:
        ...
        if 'implementation' in df.columns:
            try:
                ...
                report.append(f"Error analyzing memory efficiency data: {str(e)}\n")
        ...
        if 'implementation' in df.columns:
            ...
            if 'NumPy FFT' in df['implementation'].unique():
                try:
                    ...
                    # Use the appropriate columns for the index
                    index_cols = ['dimension', 'size']
                    # Only include dataset_idx if it exists
                    if 'dataset_idx' in df.columns:
                        index_cols.append('dataset_idx')
                    # Also include transform if it exists
                    if 'transform' in df.columns:
                        index_cols.insert(1, 'transform')  # Insert after dimension
                    ...
                    for dim in df['dimension'].unique():
                        transforms = df[df['dimension'] == dim]['transform'].unique() if 'transform' in df.columns else ['']
                        
                        for transform in transforms:
                            if 'transform' in df.columns:
                                dim_transform_indices = (pivot.index.get_level_values('dimension') == dim) & (pivot.index.get_level_values('transform') == transform)
                            else:
                                dim_transform_indices = pivot.index.get_level_values('dimension') == dim
                            
                            for impl in [col for col in pivot.columns if '_vs_NumPy' in col]:
                                impl_name = impl.replace('_vs_NumPy', '')
                                key = f"{impl_name}_{dim}D_{transform}" if transform else f"{impl_name}_{dim}D"
                    ...
                    for key, stats in speedup_stats.items():
                        ...
                        impl = parts[0]
                        dim = parts[1].replace('D', '')
                        transform = ' '.join(parts[2:]) if len(parts) > 2 else ""
                    ...
                    if 'prime_factors' in df.columns:
                        ...
                        if not prime_sizes.empty:
                            ...
                            prime_index_cols = ['dimension', 'size']
                            if 'dataset_idx' in prime_sizes.columns:
                                prime_index_cols.append('dataset_idx')
                            
                            prime_pivot = prime_sizes.pivot_table(
                                index=prime_index_cols,
                                columns='implementation',
                                values='avg_execution_time'
                            )
                    ...
                    report.append(f"Error analyzing non-power-of-two data: {str(e)}\n")
    ...
    if 'direct_comparison' in results_dict:
        ...
        time_col = 'avg_execution_time' if 'avg_execution_time' in direct_df.columns else 'time'
        
        # Check for required columns before calculating
        if 'implementation' in direct_df.columns and time_col in direct_df.columns:
            try:
                # Determine index columns for pivot
                index_cols = ['dimension', 'size']
                if 'transform' in direct_df.columns:
                    index_cols.insert(1, 'transform')
                if 'dataset_idx' in direct_df.columns:
                    index_cols.append('dataset_idx')
                ...
                report.append(f"Error calculating best implementation for general usage: {str(e)}\n")
    ...
    if 'repeated_use' in results_dict:
        ...
        if 'repetitions' in repeated_df.columns and 'implementation' in repeated_df.columns and 'amortization_factor' in repeated_df.columns:
            try:
                ...
                report.append(f"Error analyzing repeated transform data: {str(e)}\n")
    ...
    if 'thread_scaling' in results_dict:
        ...
        if 'implementation' in thread_df.columns and 'speedup' in thread_df.columns and 'thread_count' in thread_df.columns:
            try:
                ...
                report.append(f"Error analyzing thread scaling data: {str(e)}\n")
    ...
    if 'non_power_of_two' in results_dict:
        ...
        if 'implementation' in nonpow2_df.columns and 'avg_execution_time' in nonpow2_df.columns:
            try:
                # Determine index columns for pivot
                index_cols = ['dimension', 'size']
                if 'dataset_idx' in nonpow2_df.columns:
                    index_cols.append('dataset_idx')
                ...
                if 'NumPy FFT' in pivot.columns:
                    ...
                    if overall_speedup:
                        ...
                        best_impls['non_power_of_2'] = best_nonpow2
            except Exception as e:
                report.append(f"Error analyzing non-power-of-two data: {str(e)}\n")
